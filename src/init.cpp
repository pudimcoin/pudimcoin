 ...
 */
static const char* BITCOIN_PID_FILENAME = "pudimcoind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("pudimcoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/pudimcoin-project/pudimcoin>";
    const std::string URL_WEBSITE = "<https://pudimcoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("pudimcoin-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if pudimcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if pudimcoin is started while in a temporary directory.\n",
