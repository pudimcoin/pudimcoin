...
const char * const BITCOIN_CONF_FILENAME = "pudimcoin.conf";
const char * const BITCOIN_PID_FILENAME = "pudimcoin.pid";
...
#else
    const char* pszModule = "pudimcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pudimcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Pudimcoin";
#else
    // Unix
    return pathRet / ".pudimcoin";
#endif
#endif
}
